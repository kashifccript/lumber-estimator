...
export function SettingsTabs() {
  ...
  return (
    <div className="w-full">
      <div className="mb-8">
        <div className="mb-6">
          <h2 className="text-[24px] font-semibold text-[#1F1F1F] capitalize">{activeTab}</h2>
        </div>
          ...
          {tabs.map((tab) => {
            return (
              <button
                ...
                className={cn(
                  "flex cursor-pointer items-center gap-2 rounded-[5px] px-4 py-2 text-sm font-medium transition-colors w-full md:w-auto",
                  activeTab === tab.id ? "bg-[#E2624B] text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200",
                )}
              >
                {tab.icon}
                {tab.label}
